...
blockgroup_outlines <- blockgroup_outlines %>% 
	...
	select(-perc_no_degree, -perc_poverty, -median_income, -perc_renters, 
				 -perc_moved, -perc_eth_white, -perc_eth_black, -perc_eth_hispanic, 
				 -perc_eth_asian, -perc_eth_other, -perc_no_degree_sc, -perc_poverty_sc, 
				 -median_income_sc, -perc_renters_sc, -perc_moved_sc) %>% 
	glimpse()
...
crime_data <- crime_data %>% 
	mutate(
		school_time = ifelse(hour(date_single) >= 7 & hour(date_single) < 19, 
												 TRUE, FALSE),
...
crime_data <- crime_data %>% 
	mutate(public_location = case_when(
		...
		location_type %in% c("factory/warehouse", "office", "storage") ~ "private",
		TRUE ~ "other"
	)) %>% 
...
blockgroup_data <- blockgroup_data %>%
	mutate(
		period = paste(
			ifelse(school_day == TRUE, "school day", "non-school day"), 
			ifelse(school_time == TRUE, "school time", "non-school time")
		)
	)
...
# remove pre-K and K schools, closed or future schools
school_data <- school_data %>% 
	st_set_geometry(NULL) %>% 
	filter(
...
# save a copy of the school data for use in the manuscript
write_rds(school_data, "../analysis_data/school_data.Rds")
```
