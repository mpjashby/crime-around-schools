...
if (!file.exists("../original_data/crime_data.csv.gz")) {
	...
	crime_data <- dir("../original_data", pattern = "^large_city_crime_data_core_", 
										full.names = TRUE) %>% 
		...
		reduce(rbind) %>% 
		# we need five years of data so NO and VB crimes should be excluded
		filter(!city_name %in% c("New Orleans", "Virginia Beach")) %>% 
		select(
			uid, 
			city_name, 
			offense_code, 
	...
	# zip the resulting file because it's very large (>800MB)
	# zip(zipfile = "../analysis_data/crime_data.csv.zip", 
	#       files = c("../analysis_data/crime_data.csv"))
	# file.remove("../analysis_data/crime_data.csv")
...
school_holidays <- lapply(names(school_holidays), function (x) {
...
school_data <- school_locations %>% 
	...
	filter(!school_status %in% c("Closed", "Future School", "Inactive"))
...
if (!file.exists("../original_data/city_outlines.gpkg")) {
	...
	code_city_outlines <- lapply(city_data, function (x) {
		# places() from tigris
		places(
			state = x$FIPS_code,
			cb = FALSE,
...
but not open space, i.e. in [NLCD categories](https://www.mrlc.gov/nlcd11_leg.php) 22, 23 and 24.
...
if (!file.exists("../original_data/acs_data.csv")) {
	...
	acs_data <- lapply(code_counties, function (x) {
...
The crime data already contain the information necessary to construct block group ID codes, so there is no need to carry out any spatial operations.
...
crime_counts <- lapply(names(crime_types), function (x) {
	crime_data %>% 
...
crime_counts_term <- lapply(names(crime_types), function (x) {
	crime_data %>% 
	...
	reduce(rbind) %>% 
	spread(crime_type, count)
crime_counts_hldy <- lapply(names(crime_types), function (x) {
	crime_data %>% 
	...
	reduce(rbind) %>% 
	spread(crime_type, count)
crime_counts_timet <- lapply(names(crime_types), function (x) {
	crime_data %>% 
		filter(offense_code %in% crime_types[[x]] & school_day == TRUE & 
					 	school_time == TRUE) %>% 
		mutate(
	...
	reduce(rbind) %>% 
	spread(crime_type, count)
crime_counts_timef <- lapply(names(crime_types), function (x) {
	crime_data %>% 
		filter(offense_code %in% crime_types[[x]] & school_day == TRUE & 
					 	school_time == FALSE) %>% 
		mutate(
...
approximately students aged 10 and over).

```{r}
# count schools and students in each block group
school_counts <- school_data %>% 
	st_set_geometry(NULL) %>% 
	filter(school_level %in% c("High", "Middle")) %>% 
	mutate(blockgroup_id = paste0(state, county, census_tract, 
																census_block_group)) %>% 
	group_by(blockgroup_id) %>% 
	summarise(school_count = n(), school_students = sum(students))
...
# intersects the layer with itself. Note that since a cell intersects with
# itself, we need to account for this when calculating the mean crime count for
# neighbouring cells
blockgroup_data$adj <- st_intersects(blockgroup_data)

blockgroup_lags <- blockgroup_data %>% 
	# slice(1:10) %>% 
	st_intersects() %>% {
		lapply(1:length(.), function (x, y) {
			lapply(
...
blockgroup_data <- blockgroup_data %>% 
	...
	st_set_geometry(NULL)
# write file
# this uses write.csv() because of a bug with write_csv()
write.csv(blockgroup_data, "../analysis_data/blockgroup_data.csv", na = "", 
					row.names = FALSE)
# rm(blockgroup_data, blockgroup_outlines, city_data, code_city_outlines, 
#    code_counties)
```
