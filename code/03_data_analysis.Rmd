...
```{r}
f$h1_empty <- count ~ (1 | city_name/period)

f$h1_control <- count ~ count_lag_sc + (pop_total_sc + index_disadvantage + 
	index_mobility + index_ethnic + perc_teen_sc | city_name/period)

f$h1_schoolonly <- count ~ count_lag_sc + (private_elementary_school + 
	public_elementary_school + private_secondary_school + public_secondary_school
	| city_name/period)

f$h1_full <- count ~ count_lag_sc + (pop_total_sc + index_disadvantage + 
	index_mobility + index_ethnic + perc_teen_sc + private_elementary_school + 
	public_elementary_school + private_secondary_school + public_secondary_school 
	| city_name/period)

system.time(
	m$aslt$h1_schoolonly <- glmer.nb(
		f$h1_schoolonly,
		data = filter(data_h1, crime_type == "assault"),
		control = glmerControl(optCtrl = list(maxfun = 10^7), calc.derivs = FALSE)
	)
	# m$aslt$h1_schoolonly_sample_tmb <- glmmTMB::glmmTMB(
	#   f$h1_schoolonly,
	# 	data = filter(data_h1, crime_type == "assault") %>%
	# 		group_by(city_name) %>%
	# 		sample_frac(0.1) %>%
...
```{r}
ci$aslt$h1_schoolonly <- confint(m$aslt$h1_schoolonly)

paste("Finished processing data on", now())

write_rds(ci, "../analysis_data/confint.Rds")
```

```{r}
mcmc <- read_rds("../analysis_data/mcmc.Rds")
```

```{r MCMC assault school only}
mcmc$aslt$h1_schoolonly_3hour <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
	random = ~ us(1 + private_elementary_school + public_elementary_school +
...
write_rds(mcmc, "../analysis_data/mcmc.Rds")
```

```{r MCMC assault full}
mcmc$aslt$h1_full_4period <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
...
```{r}
summary(mcmc$aslt$h1_schoolonly)
```

```{r fig.height=20, fig.width=7}
par(mfrow = c(28, 2), mar = c(0.1, 0.1, 0.1, 0.1))
plot(mcmc$aslt$h1_schoolonly, ask = FALSE, auto.layout = FALSE)
```

```{r}
mcmc$aslt$h1_schoolonly_gelman <- parallel::mclapply(1:4, function (i) {
	MCMCglmm::MCMCglmm(
		fixed = count ~ count_lag_sc,
		random = ~ us(1 + private_elementary_school + public_elementary_school +
										private_secondary_school + 
...
mcmc$aslt$h1_schoolonly_gelman %>% 
...
```{r Distribution chart, fig.height=9.21, fig.width=6.85}
...
plot_data <- as.tibble(mcmc$aslt$h1_full_4period$Sol) %>% 
	gather() %>% 
...
plot_x_axis_lims <- ggplot_build(dist_plot)$layout$panel_params[[1]]$x.range
