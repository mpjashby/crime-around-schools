...
activities cause increases in crime, we would expect:
  ...
  * H4: in neighbourhoods with schools, more high/middle-school students will be 
    associated with more violence at school times on school days but not on 
    vacation weekdays or at at non-school times.
...
```{r}
data <- read_rds(here::here("analysis_data/blockgroup_data.Rds")) %>% 
	glimpse()
```
...
```{r}
data_h1 <- data %>% 
	filter(!city_name %in% c("Fort Worth") & school_time == TRUE) %>%
...
plot_data <- mcmc_h1$robb_full %>% 
	tidy_draws() %>% 
	mean_hdci() %>% 
	gather(key = "variable", value = "value") %>% 
	mutate(
		stat = case_when(
			str_sub(variable, -5) == "lower" ~ "lower_hdi",
			str_sub(variable, -5) == "upper" ~ "upper_hdi",
			TRUE ~ "mean"
		),
		variable = ifelse(
			str_sub(variable, -5) %in% c("lower", "upper"),
			str_sub(variable, 0, -7),
			variable
		)
	) %>% 
	spread(stat, value) %>% 
	separate(variable, c("term", "city_period"), ".city_period.") %>% 
	separate(city_period, c("city", "period"), coll(" - ")) %>%
	# filter(!str_detect(term, coll("VCV_"))) %>% 
	...
	mutate_at(vars(one_of("mean", "lower_hdi", "upper_hdi")), as.numeric) %>% 
	mutate_at(vars(one_of("mean", "lower_hdi", "upper_hdi")), exp) %>% 
	remove_missing(na.rm = TRUE) %>% 
	arrange(city, term, period) %>% 
	mutate(
		diff = ifelse(
			!is.na(lag(term)) & lag(term) == term,
			ifelse(
				lag(mean) > mean,
				ifelse(lag(lower_hdi) > upper_hdi, TRUE, FALSE),
				ifelse(lag(upper_hdi) < lower_hdi, TRUE, FALSE)
			),
			ifelse(
				lead(mean) > mean,
				ifelse(lead(lower_hdi) > upper_hdi, TRUE, FALSE),
				ifelse(lead(upper_hdi) < lower_hdi, TRUE, FALSE)
			)
		),
		label = ifelse(
			period == "school day school time", 
			paste0(format(mean, digits = 2), "\n"), 
			""
		),
		min = ifelse(mean > 1, lower_hdi, upper_hdi),
		period = str_remove(period, " school time$"),
		sig = ifelse(
			mean > 1,
			ifelse(lower_hdi > 1, TRUE, FALSE),
			ifelse(upper_hdi < 1, TRUE, FALSE)
		),
		sig = case_when(
			diff == TRUE & sig == TRUE ~ "diff sig",
			diff == TRUE & sig == FALSE ~ "diff non-sig",
			diff == FALSE & sig == TRUE ~ "same sig",
			diff == FALSE & sig == FALSE ~ "same non-sig",
			TRUE ~ NA_character_
		),
		term = str_replace(str_replace_all(str_sub(term, 0, -5), "_", " "), 
											 "^(\\w+?) (.+)$", "\\2, \\1")
	)

plot_cities <- plot_data %>% 
	filter(term == "secondary school, public" & period == "school day") %>%
	arrange(desc(mean)) %>%
	pluck("city")
...
plot_data %>% 
  ...
  ggplot() + 
	geom_segment(aes(x = period, xend = period, y = 1, yend = min), 
							 colour = "#999999", linetype = "12") +
	geom_hline(yintercept = 1, colour = "grey60", size = 0.25) +
	geom_segment(aes(x = period, xend = period, y = lower_hdi, yend = upper_hdi, 
											colour = sig)) +
	geom_point(aes(x = period, y = mean, colour = sig, fill = sig), shape = 21, 
						 size = 2.5) +
	geom_text(aes(x = period, y = mean, label = label), lineheight = 0.9, 
						size = 2.5, vjust = "bottom") +
	scale_y_continuous(expand = c(0, 0), trans = "log", 
										 limits = c(1/3.3333, 3.33333), breaks = c(0.5, 1.0, 2.0)) +
	scale_colour_manual(
		values = c(
			"diff sig" = "#000000",
			"diff non-sig" = "#999999",
		...
		labels = c(
			"diff sig" = "different from intercept and from other period  ",
			"diff non-sig" = "different from other period but not from intercept  ",
			"same sig" = "different from intercept but not from other period  ",
			"same non-sig" = "not different from intercept and not from other period  "
		),
	...
	labs(y = "odds ratio (natural log scale)") +
	facet_grid(rows = vars(city), cols = vars(term)) +
	coord_flip() +
	theme_minimal() +
	theme(
...
data_h3 <- data %>%
	...
	mutate(
		count_lag_sc = as.numeric(scale(count_lag, center = TRUE, scale = TRUE)),
		city_period = as.factor(paste0(city_name, " - ", public_location))
	) %>% 
	glimpse()
```
...
```{r H3 assault empty}
mcmc_h3$aslt_empty <- MCMCglmm::MCMCglmm(
	fixed = count ~ 1,
	random = ~ us(1):city_period,
...
mcmc_h3$aslt_control <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
	random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
									index_ethnic + perc_teen_sc):city_period,
	data = filter(as.data.frame(data_h3), crime_type == "assault"),
...
```{r H3 assault full}
mcmc_h3$aslt_full <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
	random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
									index_ethnic + perc_teen_sc + private_elementary_school +
									public_elementary_school + private_secondary_school + 
...
## Create data for testing H4

We can recycle the data used for H1 and H2, but to test the H4 we must exclude 
block groups without public schools. Private schools cannot be included because
there are not enough of them.

```{r}
data_h4 <- data_h1 %>% 
	filter(public_elementary_school == TRUE | public_secondary_school == TRUE) %>% 
	mutate(
		public_elementary_students_sc = as.numeric(scale(
			public_elementary_students, center = FALSE, scale = TRUE)),
		public_secondary_students_sc = as.numeric(
			scale(public_secondary_students, center = FALSE, scale = TRUE))
	) %>% 
	glimpse()
...
```{r}
mcmc_h4 <- read_rds("../analysis_data/mcmc_h4.Rds")
```


```{r H4 assault empty}
mcmc_h4$aslt_empty <- MCMCglmm::MCMCglmm(
	fixed = count ~ 1,
	random = ~ us(1):city_period,
	data = filter(as.data.frame(data_h4), crime_type == "assault"),
	family = "poisson", # MCMC deals with over-dispersion
	prior = list(
		R = list(V = diag(1), nu = 0.002),
		G = list(
...
write_rds(mcmc_h4, "../analysis_data/mcmc_h4.Rds")
```


```{r H4 assault control}
mcmc_h4$aslt_control <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
	random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
									index_ethnic + perc_teen_sc):city_period,
	data = filter(as.data.frame(data_h4), crime_type == "assault"),
...
```{r H4 assault full}
mcmc_h4$aslt_full <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
	random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
									index_ethnic + perc_teen_sc + public_elementary_students_sc +
									public_secondary_students_sc):city_period,
	data = filter(as.data.frame(data_h4), crime_type == "assault"),
	family = "poisson", # MCMC deals with over-dispersion
	prior = list(
		R = list(V = diag(1), nu = 0.002),
		G = list(
			G1 = list(V = diag(8), n = 8)
		)
	),
	thin = 20,
	burnin = 5000,
...
```{r H4 assault compare models}
# this code from https://stackoverflow.com/a/31484507/8222654
MuMIn::model.sel(mcmc_h4$aslt_empty, mcmc_h4$aslt_control, mcmc_h4$aslt_full, 
								 rank = "DIC")
```


```{r H4 robbery empty}
mcmc_h4$robb_empty <- MCMCglmm::MCMCglmm(
	fixed = count ~ 1,
	random = ~ us(1):city_period,
	data = filter(as.data.frame(data_h4), crime_type == "robbery", 
								!city_name %in% c("Kansas City", "Virginia Beach")),
	family = "poisson", # MCMC deals with over-dispersion
	prior = list(
		R = list(V = diag(1), nu = 0.002),
...
```{r H4 robbery control}
mcmc_h4$robb_control <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
	random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
									index_ethnic + perc_teen_sc):city_period,
	data = filter(as.data.frame(data_h4), crime_type == "robbery", 
...
```{r H4 robbery full}
mcmc_h4$robb_full <- MCMCglmm::MCMCglmm(
	fixed = count ~ count_lag_sc,
	random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
									index_ethnic + perc_teen_sc + public_elementary_students_sc +
									public_secondary_students_sc):city_period,
...
```{r H4 robbery compare models}
# this code from https://stackoverflow.com/a/31484507/8222654
MuMIn::model.sel(mcmc_h4$robb_empty, mcmc_h4$robb_control, mcmc_h4$robb_full, 
								 rank = "DIC")
```


```{r H4 Gelman tests}
mcmc_h4$aslt_full_gelman <- parallel::mclapply(1:4, function (i) {
	message("Starting iteration ", i, appendLF = TRUE)
	MCMCglmm::MCMCglmm(
		fixed = count ~ count_lag_sc,
		random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
...
mcmc_h4$robb_full_gelman <- parallel::mclapply(1:4, function (i) {
	message("Starting iteration ", i, appendLF = TRUE)
	MCMCglmm::MCMCglmm(
		fixed = count ~ count_lag_sc,
		random = ~ us(1 + pop_total_sc + index_disadvantage + index_mobility + 
...
```{r H4 distribution chart, fig.height=9.21, fig.width=6.85}
...
plot_data <- pluck(mcmc_h4$robb_full, "Sol") %>% 
	as.tibble() %>% 
	gather() %>% 
	separate(key, c("term", "city_period"), ".city_period.") %>% 
	separate(city_period, c("city", "period"), coll(" - ")) %>%
	separate(period, c("day", "time"), coll(" day ")) %>% 
	filter(term %in% c("public_elementary_students_sc", 
										 "public_secondary_students_sc")) %>% 
	remove_missing(na.rm = TRUE) %>% 
	mutate(
		term = str_replace_all(str_sub(term, 0, -4), "_", " "),
		period = paste(day, ifelse(time == "school time", "day", "night")),
		period = factor(period, levels = c("school day", "non-school day", 
																			 "school night", "non-school night")),
		value = exp(value)
...
plot_cities <- plot_means %>% 
	filter(term == "public secondary students") %>% 
	arrange(desc(mean_value)) %>% 
	pluck("city")
...
newdata <- expand.grid(
	count = 0,
	# since all the control variables were centered and scaled, they can be set to
	# zero here to represent block groups with the mean values
	count_lag_sc = 0,
	pop_total_sc = 0,
	perc_teen_sc = 0,
	index_disadvantage = 0,
	index_mobility = 0,
	# the ethnic heterogeneity index is not centered or scaled, so a value of 0.5
	# is used because it approximately represents the mean value across cities
	index_ethnic = 0.5,
	# since we know there is only a meaningful effect for public secondary 
	# schools, we can set all the other school indicators to FALSE
	...
	# public_elementary_students_sc = 0,
	# public_secondary_students_sc = seq(0, 100, by = 10),
	# since we're interested in comparing block groups with and without schools,
	# we only need to get predictions for school times on school days
	city_period = c(
		"Chicago - non-school day school time",
		"Chicago - school day school time", 
		"Detroit - non-school day school time",
		"Detroit - school day school time",
		"Kansas City - non-school day school time",
		"Kansas City - school day school time",
		"Los Angeles - non-school day school time",
		"Los Angeles - school day school time",
		"Louisville - non-school day school time",
		"Louisville - school day school time",
		"New York - non-school day school time",
		"New York - school day school time",
		"San Francisco - non-school day school time",
		"San Francisco - school day school time",
		"Tucson - non-school day school time",
		"Tucson - school day school time",
		"Virginia Beach - non-school day school time",
		"Virginia Beach - school day school time"
	)
)

predictions <- predict(mcmc_h1$aslt_full, newdata = newdata, marginal = NULL, 
											 type = "response", posterior = "mean", verbose = TRUE)
```
